...
Page({
  data:{
    dateText:'',greeting:'',homeTitle:'今天想吃点什么呀？',
    homeSubtitle:'认真选一顿，也是在认真过日子。',recommendations:[],recent:null,
  ...
  async bootstrap(){
    ...
    this.setData({
      ...
      greeting:h<11?'早上好':h<17?'下午好':'晚上好',loading:true,error:''
    })
    try{
      await getApp().loginPromise
      const user=wx.getStorageSync('user')
      if(!user?.coupleId){this.setData({needsCouple:true,loading:false});return}
      ...
      this.setData({
        homeTitle:couple.homeTitle||'今天想吃点什么呀？',
        homeSubtitle:couple.homeSubtitle||'认真选一顿，也是在认真过日子。',
